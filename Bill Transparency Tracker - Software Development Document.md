...
**Version:** 0.1.1 (Baseline app scaffold implemented)
**Status:** In implementation (core v1 scaffold complete)
**Owner:** Tony
...
- **Bootstrap 5 usage:** use Bootstrap 5 via CDN (simplest for a solo project). Current implementation pins `5.3.3` and applies a custom `theme.css` that overrides Bootstrap CSS variables (`--bs-primary`, `--bs-danger`, etc.) to the palette above rather than using heavy `!important` overrides.
...
4. Admin auth mechanism — **resolved for v1 scaffold** as single-owner HTTP Basic auth (`ADMIN_USERNAME` / `ADMIN_PASSWORD`). Revisit later if session auth is needed.
...
### 15.1 Resolved during baseline scaffold (2026-07-14)
1. Admin auth mechanism resolved for v1 scaffold: HTTP Basic auth using owner credentials from environment variables (`ADMIN_USERNAME`, `ADMIN_PASSWORD`).
